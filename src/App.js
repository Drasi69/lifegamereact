...
export default function Home() {
  ...
  const [world, setWorld] = useState(
    Array(20)
      .fill(null)
      .map((row) => new Array(30).fill(null))
  );
  ...
  useEffect(() => {
    const newWorld = Array(maxRow)
      .fill(null)
      .map((row) => new Array(maxCol).fill(null));
    setWorld(newWorld);
    setGeneration(0);
  }, [maxRow, maxCol]);
  ...
  const getNeighbours = useCallback((world, row, col, maxRow, maxCol) => {
    ...
    if (row > 0) {
      if (col > 0) {
        if (world[row - 1][col - 1]) num++;
      }
      if (world[row - 1][col]) num++;
      if (col < maxCol - 1) {
        if (world[row - 1][col + 1]) num++;
      }
    }
    if (col > 0) {
      if (world[row][col - 1]) num++;
    }
    if (col < maxCol - 1) {
      if (world[row][col + 1]) num++;
    }
  ...
  const randomFill = useCallback(() => {
    const newWorld = Array(maxRow)
      .fill(null)
      .map((row) => new Array(maxCol).fill(null).map(() => (Math.random() > 0.7 ? 'X' : null)));
    setWorld(newWorld);
    setGeneration(0);
  }, [maxRow, maxCol]);
  ...
  return (
    <main>
      ...
      <div className="controls-section">
        ...
        <div className="control-group">
          <h3>Controls</h3>
          <button className="button-3" onClick={start} disabled={isRunning}>
            Start
          </button>
          <button className="button-3" onClick={stop} disabled={!isRunning}>
            Stop
          </button>
          <button className="button-3" onClick={clearGrid} disabled={isRunning}>
            Clear
          </button>
          <button className="button-3" onClick={randomFill} disabled={isRunning}>
            Random
          </button>
        </div>
      </div>
